...
def UPV_scraper(year, month, day, dest_dir, ifo, verbose = False):
    ...
    for k in range(len(source_channel)):
        ...
        #pdb.set_trace()
        # ****************************
        # PROBLEM HERE with UPV_dest_path_seg_file_winner (line 203) and veto_file (lines 206-207)
        # ****************************
...
def query_cwb_events(dest_dir, ifo, year, month, day, verbose = False):
    ...
    :param dest_dir: destination directory (str)
    :param ifo: ifo, e.g., H1 or L1 (str)
    :param year: year (int)
    :param month: month (int)
    :param day: day (int)
    :return: None
    ...
    cmd = f'scp ldas-pcdev2.ligo.caltech.edu:{cwb_base_dir} {dest_dir}'
...
def Write_config(dest_dir, config_file, IFO, flag_name1, flag_file1, flag_name2, flag_file2, paddings, UPV_channel_names, hveto_channel_names, verbose = False):
    ...
    :param flag_file1: name of the UPV flag file, which is expected to be an .xml file
    :param flag_name2: flag name in the hveto flag
    :param flag_file2: name of the hveto flag file, which is expected to be an .xml file
    :param paddings: tupple or list of the paddings, ([pre_pad, post_pad], ....), e.g., ([1, 0], [0, 1], [1, 1])
    :param UPV_channel_names: array of the UPV channels for the day (array)
    :param hveto_channel_names: array of the Hveto channels for the day (array)
    :return: None
...
def Wrapper(ifo, year, month, day, dest_dir = '.', verbose = False):
    ...
    month       = month (int)
    day         = day (int)
    dest_dir    = (optional) destination directory, default = '.' (str)
    ...
    if isinstance(day, int):
        ...
        if os.path.exists(file):
            ...
            if True:#__name__ == '__main__':
                ...
                if verbose:
                    print(f"-> Done (UPV_sorted.txt file for each round)")
                ...
                if verbose:
                    print(f"-> Done (hveto_sorted.txt file for each round)")
                ...
                for n in UPV_flag_name:
                    ...
                    if verbose:
                        if n == 0:
                            print(f"\t-- xml file for all data created\n")
                        else:
                            print(f"\t-- xml file for round {n} created\n")

                if verbose:
                    print(f"-> Done (flag xml file for all rounds of UPV)")


                if verbose:
                    print(f"\n|| Making flag xml file for hveto...\n")
                ...
                for m in hveto_flag_name:
                    ...
                    if verbose:
                        if m == 0:
                            ...
                            print(f"\t-- xml file for round {m} created\n")

                if verbose:
                    print(f"-> Done (flag xml file for all rounds of hveto)")
